...
if (!empty($_GET['edit'])) {
    $id = (int)$_GET['edit'];
    $stmt = $pdo->prepare("SELECT * FROM seasonals WHERE id = ?");
    $stmt->execute([$id]);
    $fetched = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($fetched) {
        $item = $fetched;
        $is_edit = true;
        $page_title = "Edit Seasonal Theme";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        $uploadDir = __DIR__ . '/../assets/uploads/seasonals/';
    ...
    if (empty($errors)) {
        try {
            if ($is_edit) {
                $stmt = $pdo->prepare("UPDATE seasonals SET title=?, slug=?, description=?, image=? WHERE id=?");
                $stmt->execute([$title, $slug, $desc, $imagePath, $id]);
                $_SESSION['success_msg'] = "Seasonal theme updated successfully.";
            } else {
                $stmt = $pdo->prepare("INSERT INTO seasonals (title, slug, description, image) VALUES (?, ?, ?, ?)");
                $stmt->execute([$title, $slug, $desc, $imagePath]);
                $_SESSION['success_msg'] = "Seasonal theme added successfully.";
            }
...
<div class="max-w-[800px] mx-auto py-6">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-extrabold text-slate-800"><?= $is_edit ? 'Edit Seasonal Theme' : 'Add New Seasonal Theme' ?></h2>
        <a href="seasonals.php" class="px-4 py-2 rounded-lg border border-gray-300 bg-white hover:bg-gray-50">Cancel</a>
    </div>
    ...
    <div class="bg-white border border-gray-200 p-6 rounded-xl shadow-sm">
        <form method="POST" enctype="multipart/form-data">
                ...
                <?php 
                    ...
                    if ($item['image']) {
                        ...
                        if (strpos($item['image'], '/') !== false) {
                            ...
                            $displayImg = '/assets/uploads/seasonals/' . $item['image'];
                        }
                    }
                ?>
            ...
            <button type="submit" class="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg shadow-lg">
                <?= $is_edit ? 'Update Seasonal Theme' : 'Create Seasonal Theme' ?>
            </button>
        </form>
    </div>
</div>
