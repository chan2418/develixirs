<?php
// admin/add_product.php - upgraded to new admin layout & style
...
// Helper to repopulate value
function old_val($field, $default = '') {
    global $old;
    return isset($old[$field]) ? htmlspecialchars($old[$field], ENT_QUOTES, 'UTF-8') : htmlspecialchars($default, ENT_QUOTES, 'UTF-8');
}
...
$categories = [];
try {
    $categories = $pdo->query("SELECT id, COALESCE(title, name) AS title FROM categories ORDER BY COALESCE(title, name) ASC")->fetchAll(PDO::FETCH_ASSOC);
} catch(Exception $e) {
...
<div class="max-w-[1200px] mx-auto py-6">
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <p class="text-sm text-slate-500 mt-1">Create a product entry — upload images and fill SEO fields. Errors and previous form values will be shown below if any.</p>
    </div>
  ...
  <form action="save_product.php" method="post" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 bg-transparent">
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
      ...
      <div class="mb-4">
        ...
        <input id="title" name="title" required class="w-full p-3 border rounded-lg" value="<?php echo $name_val; ?>" placeholder="E.g. Classic Leather Sneakers">
      </div>

      <div class="mb-4">
        <label class="block text-sm font-semibold mb-2">Short Description</label>
        <input id="short_desc" name="short_desc" class="w-full p-3 border rounded-lg" value="<?php echo $short_desc_val; ?>" placeholder="One-line summary">
      </div>

      <div class="mb-4">
        <label class="block text-sm font-semibold mb-2">Full Description</label>
        <textarea id="description" name="description" rows="6" class="w-full p-3 border rounded-lg" placeholder="Full product description"><?php echo $description_val; ?></textarea>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-semibold mb-2">SEO Title</label>
          <input id="meta_title" name="meta_title" class="w-full p-3 border rounded-lg" value="<?php echo $meta_title_val; ?>">
        </div>
        <div>
          <label class="block text-sm font-semibold mb-2">SEO Description</label>
          <input id="meta_description" name="meta_description" class="w-full p-3 border rounded-lg" value="<?php echo $meta_description_val; ?>">
        </div>
      </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
      ...
      <div>
        ...
        <select id="category_id" name="category_id" class="w-full p-3 border rounded-lg">
          ...
          <?php foreach($categories as $c): ?>
            <option value="<?php echo (int)$c['id']; ?>" <?php echo ((string)$category_id_val === (string)$c['id']) ? 'selected' : ''; ?>>
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input id="price" name="price" type="number" step="0.01" min="0" class="w-full p-3 border rounded-lg" value="<?php echo $price_val; ?>" required>
        </div>
        <div>
          <label class="block text-sm font-semibold mb-2">Stock</label>
          <input id="stock" name="stock" type="number" min="0" class="w-full p-3 border rounded-lg" value="<?php echo $stock_val; ?>" required>
        </div>
      </div>

      <div>
        <label class="block text-sm font-semibold mb-2">SKU</label>
        <input id="sku" name="sku" class="w-full p-3 border rounded-lg" value="<?php echo $sku_val; ?>" placeholder="Optional SKU">
      </div>
      ...
      <div>
        ...
        <p class="text-xs text-slate-500 mt-2">Tip: Uploaded files will be handled by <code>save_product.php</code>.</p>
      </div>
