...
if (!empty($_GET['id'])) {
    ...
    // error_log("Edit Herbal ID: $id - Fetched Data: " . print_r($fetched, true));
    ...
    if ($fetched) {
        ...
        $_SESSION['error_msg'] = "Herbal category not found.";
        header("Location: herbals.php");
        exit;
    }
} else {
    // If we are strictly edit page, redirect. If reused for add, handle differently
    // But since we split, this page expects ID.
    // Wait, let's keep it safe. If no ID, redirect to herbals or show error?
    // The user split the files, so this IS edit_herbal.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Image Upload
    $imagePath = $herbal['image']; // keep existing
...
<div class="max-w-[800px] mx-auto py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-extrabold text-slate-800">Edit Herbal Category</h2>
    ...
    <div class="bg-white border border-gray-200 p-6 rounded-xl shadow-sm">
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" value="<?= htmlspecialchars($herbal['title']) ?>" class="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-indigo-100 outline-none" required>
            </div>

            <!-- Slug -->
            <div class="mb-4">
                <label class="block text-sm font-semibold text-gray-700 mb-1">Slug (Optional)</label>
                <input type="text" name="slug" value="<?= htmlspecialchars($herbal['slug']) ?>" class="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-indigo-100 outline-none" placeholder="auto-generated-from-title">
            </div>

            <!-- Description -->
            <div class="mb-4">
                <label class="block text-sm font-semibold text-gray-700 mb-1">Description</label>
                <textarea name="description" rows="3" class="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-indigo-100 outline-none"><?= htmlspecialchars($herbal['description']) ?></textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <?php 
                    ...
                    if ($herbal['image']) {
                        ...
                        if (strpos($herbal['image'], 'http') === 0 || strpos($herbal['image'], '/') === 0) {
                            $displayImg = $herbal['image'];
                        } else {
                            $displayImg = '/assets/uploads/herbals/' . $herbal['image'];
                        }
                    }
                ?>
