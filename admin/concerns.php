...
try {
    ...
    if ($stmt->rowCount() == 0) {
        $pdo->exec("ALTER TABLE products ADD COLUMN concern_id INT NULL DEFAULT NULL");
    }
} catch (Exception $e) {
    // Silent fail or log, but page might break if table missing
    error_log("Concern DB Setup Error: " . $e->getMessage());
}
// -----------------------------

$page_title = "Shop by Concern";
...
<div class="max-w-[1200px] mx-auto py-6">
    ...
    <div class="flex items-start justify-between mb-6">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-800">Shop by Concern</h2>
            <p class="text-sm text-slate-500">Manage health concerns for product filtering.</p>
        </div>

        <div class="flex gap-3">
            <a href="products.php" class="px-4 py-2 rounded-lg border border-gray-300 bg-white hover:bg-gray-50">← Back to Products</a>
            <a href="add_concern.php" class="px-4 py-2 rounded-lg bg-green-600 text-white font-semibold hover:bg-green-700 shadow shadow-green-200">+ Add Concern</a>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 p-6 rounded-xl shadow-sm">
        ...
        <?php if(empty($concerns)): ?>
            <div class="text-center py-10 text-gray-400">
                ...
                <p>No concerns found. Add one to get started.</p>
            </div>
        <?php else: ?>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        <?php foreach($concerns as $c): 
                            ...
                            if(!empty($c['image'])) {
                                $imgSrc = '/assets/uploads/concerns/' . ltrim($c['image'], '/'); 
                            }
                        ?>
                        <tr class="border-b hover:bg-gray-50 transition">
                            ...
                            <td class="p-3 font-semibold text-slate-700"><?= htmlspecialchars($c['title']) ?></td>
                            <td class="p-3 text-gray-500"><?= htmlspecialchars($c['slug']) ?></td>
                            <td class="p-3">
                                <div class="flex gap-3">
                                    <a href="add_concern.php?edit=<?= $c['id'] ?>" class="text-indigo-600 font-semibold hover:text-indigo-800">Edit</a>
                                    <a href="delete_concern.php?id=<?= $c['id'] ?>" onclick="return confirm('Delete this concern?');" class="text-red-600 font-semibold hover:text-red-800">Delete</a>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
