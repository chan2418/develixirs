...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            if ($is_edit) {
                $stmt = $pdo->prepare("UPDATE concerns SET title=?, slug=?, description=?, image=? WHERE id=?");
                $stmt->execute([$title, $slug, $desc, $imagePath, $id]);
                $_SESSION['success_msg'] = "Concern updated successfully.";
            } else {
                $stmt = $pdo->prepare("INSERT INTO concerns (title, slug, description, image) VALUES (?, ?, ?, ?)");
                $stmt->execute([$title, $slug, $desc, $imagePath]);
                $_SESSION['success_msg'] = "Concern added successfully.";
            }
...
<div class="max-w-[800px] mx-auto py-6">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-extrabold text-slate-800"><?= $is_edit ? 'Edit Concern' : 'Add New Concern' ?></h2>
        <a href="concerns.php" class="px-4 py-2 rounded-lg border border-gray-300 bg-white hover:bg-gray-50">Cancel</a>
    </div>
    ...
    <div class="bg-white border border-gray-200 p-6 rounded-xl shadow-sm">
        <form method="POST" enctype="multipart/form-data">
                ...
                <?php 
                    ...
                    if ($item['image']) {
                        ...
                        if (strpos($item['image'], '/') !== false) {
                            ...
                            $displayImg = '/assets/uploads/concerns/' . $item['image'];
                        }
                    }
                ?>
            ...
            <button type="submit" class="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg shadow-lg">
                <?= $is_edit ? 'Update Concern' : 'Create Concern' ?>
            </button>
        </form>
    </div>
</div>
