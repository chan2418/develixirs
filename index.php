...
$herbals = [];
try {
    // Check if table exists first to avoid error on fresh page load 
    // (Admin page creates it, but good to be safe)
    $stmt = $pdo->query("SHOW TABLES LIKE 'herbals'");
    if($stmt->rowCount() > 0) {
...
function get_product_tags($pdo, $pid) {
    if (!$pid) return [];
    try {
        $stmt = $pdo->prepare("
            SELECT t.name 
...
try {
    ...
    $bannerGroups = [];
    try {
        $stmt = $pdo->query("SELECT * FROM product_groups ORDER BY id DESC");
        $bannerGroups = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // Fetch "Picks" products
        $stmtPicks = $pdo->prepare("
            SELECT p.id, p.name, p.price, p.images 
            FROM homepage_products hp
            JOIN products p ON hp.product_id = p.id
            WHERE hp.section = 'picks' AND p.is_active = 1
            ORDER BY hp.sort_order ASC
        ");
        $stmtPicks->execute();
        $picksProducts = $stmtPicks->fetchAll(PDO::FETCH_ASSOC);
        
        // Settings for Picks
        $showPicks = isset($homeSettings['show_picks_products']) ? $homeSettings['show_picks_products'] : '1';
        $titlePicks = !empty($homeSettings['title_picks_products']) ? $homeSettings['title_picks_products'] : 'DevElixirs Picks';

        // Fallback if empty and visible (optional: keep using latest 2)
        if (empty($picksProducts) && $showPicks) {
    ...
    $newProducts = fetch_home_section($pdo, 'new_herbal', 3);

    // Fetch tags explicitly for New Products
    foreach ($newProducts as &$np) {
        $np['tags'] = get_product_tags($pdo, $np['id']);
    }
    unset($np);


    // Devilixirs Picks - Logic moved to DB fetch block above
    // $picksProducts = array_slice($allProducts, 0, 2);
    
    // Fetch tags for picks 
    if (!empty($picksProducts)) {
        foreach ($picksProducts as &$pp) {
             $pp['tags'] = get_product_tags($pdo, $pp['id']);
        }
    }
    unset($pp);
    ...
    foreach ($tabLatest as &$p) {
        $stmtTags = $pdo->prepare("SELECT t.name FROM tags t JOIN product_tags pt ON pt.tag_id = t.id WHERE pt.product_id = :pid ORDER BY t.name ASC LIMIT 2");
        $stmtTags->execute([':pid' => $p['id']]);
        $p['tags'] = $stmtTags->fetchAll(PDO::FETCH_COLUMN);
    }
    unset($p);
     ...
     */
    $bestProducts = fetch_home_section($pdo, 'best_seller', 3);

    foreach ($bestProducts as &$bp) {
        $stmtTags = $pdo->prepare("SELECT t.name FROM tags t JOIN product_tags pt ON pt.tag_id = t.id WHERE pt.product_id = :pid ORDER BY t.name ASC LIMIT 2");
        $stmtTags->execute([':pid' => $bp['id']]);
        $bp['tags'] = $stmtTags->fetchAll(PDO::FETCH_COLUMN);
    }
    unset($bp);
    ...
    $categories = $allProducts = $newProducts = $bestProducts = [];
    $picksProducts = [];
...
?>
  ...
  cursor:pointer;
    ...
    /* Bigger cards for New Herbal Products (3 items) */
    .new-herbal-grid {
      grid-template-columns: repeat(3, minmax(0, 1fr));
    }
    .new-herbal-grid .product-card {
      min-height: 420px; /* Increase height */
    }
    .new-herbal-grid .product-image img {
    ...
    .product-tags {
      ...
      gap: 4px;
      margin-bottom: 5px;
      min-height: 20px; /* Use min-height instead */
      height: auto;
    ...
    /* Large Screens: Sidebar Sticky */
    @media (min-width: 993px) {
      .page-grid {
        /* Ensure the grid aligns top so sticky works */
        align-items: start;
      }
      .side-column {
        position: -webkit-sticky; /* Safari */
        position: sticky;
        top: 90px; /* Adjust based on header height (approx 70px + 20px gap) */
        height: fit-content;
        z-index: 9;
      }
    }
    ...
    @media (max-width:768px){
      ...
      /* Mobile: Horizontal scroll with Auto-Scroll (per user request) */
      .product-grid{
      ...
      /* Mobile Fix for New Herbal Products (prevent stretching) */
      .new-herbal-grid .product-card {
        min-height: auto !important; 
        height: auto !important;
      }
      .new-herbal-grid .product-image img {
        height: 160px !important; /* Make it square-ish to match width */
      }
      .top-bar-inner{
        flex-direction:column;
        align-items:flex-start;
    ...
    @media (max-width:768px){
      /* Hide sidebar Categories on mobile (redundant with main shop-by-category) */
      .categories-card {
        display: none !important;
      }
      
      /* Ensure other sidebar items are visible/styled */
      .side-column {
         display: flex !important;
         flex-direction: column;
         gap: 20px;
...
<body>
  ...
  <?php endif; ?>
          ...
          <?php if ($showPicks !== '0'): ?>
          <div class="card desktop-picks">
            <div class="card-header">
              <div class="label">
                <i class="fa-solid fa-trophy"></i>
                <span><?php echo htmlspecialchars($titlePicks); ?></span>
              </div>
            </div>
            <div class="card-body">
          ...
          <div class="card promo-card">
            ...
            <?php else: ?>
              <!-- Fallback static image using same style -->
               <div class="promo-image">
                  <img
                    src="https://dove.themeftc.com/wp-content/uploads/2021/05/bn-left20.jpg"
                    alt="Offer">
        ...
        <div class="main-column">
          ...
          <section class="content-grid">
            ...
            <div class="product-grid new-herbal-grid">
                      ...
                      <?php if (!empty($p['tags'])): ?>
                        <div class="product-tags">
                          <?php foreach (array_slice($p['tags'], 0, 2) as $t): ?>
                            <span class="product-tag"><?php echo htmlspecialchars($t); ?></span>
                          <?php endforeach; ?>
                        </div>
                      <?php else: ?>
                        <!-- Spacer to keep alignment if needed, or empty -->
            ...
            <!-- MOBILE ONLY: DEVILIXIRS PICKS (Moved here for mobile view) -->
            <?php if ($showPicks !== '0'): ?>
            <div class="mobile-picks-section" style="margin-top: 30px; display: none;">
              <div class="section-header">
                <h3><?php echo htmlspecialchars($titlePicks); ?></h3>
              </div>
              <div class="card" style="border:1px solid #e1e1e1; border-radius:8px; padding:0;">
                <div class="card-body" style="padding:10px;">
                  <?php if (!empty($picksProducts)): ?>
                    <?php foreach ($picksProducts as $p): ?>
                      <?php $img = htmlspecialchars(get_first_image($p['images'] ?? '')); ?>
                      <a href="product_view.php?id=<?php echo (int)$p['id']; ?>" class="mini-product" style="text-decoration:none; color:inherit; display:flex; align-items:center; gap:12px; margin-bottom:12px; border-bottom:1px solid #eee; padding-bottom:12px;">
                        <img src="<?php echo $img; ?>" alt="Product" style="width:60px; height:60px; object-fit:cover; border-radius:4px;">
                        <div class="mini-info">
                          <div class="mini-name" style="font-size:13px; font-weight:500; margin-bottom:4px;">
                            <?php echo htmlspecialchars($p['name']); ?>
                          </div>
                          <div class="product-stars" style="font-size:10px; color:#ccc; margin-bottom:4px;">★★★★★</div>
                          <div class="mini-price" style="font-size:13px; font-weight:600; color:#b89026;">
                            ₹<?php echo number_format((float)$p['price'], 2); ?>
                          </div>
                        </div>
                      </a>
            ...
            <style>
              @media (max-width: 768px) {
                ...
                .mobile-picks-section { display: block !important; }
              }
            </style>
            ...
            <div class="top-concerns-section" style="margin-top: 40px; margin-bottom: 20px;">
              ...
              <div id="concernAutoScroll" style="display: flex; gap: 30px; overflow-x: auto; padding: 10px 4px; -webkit-overflow-scrolling: touch;">
                ...
                ?>
                  <a href="product.php?concern=<?php echo urlencode($c['slug']); ?>" style="flex: 0 0 calc(33.333% - 20px); text-decoration: none; color: inherit; display: flex; flex-direction: column; align-items: center; gap: 12px; scroll-snap-align: start;">
                    <div style="width: 100%; aspect-ratio: 1 / 1; border-radius: 50%; overflow: hidden; border: 3px solid #fff; box-shadow: 0 4px 10px rgba(0,0,0,0.1); background: #fff; position: relative; padding: 10px;">
                         <img src="<?php echo htmlspecialchars($cImg, ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars($c['title'], ENT_QUOTES, 'UTF-8'); ?>" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%; transition: transform 0.3s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                    </div>
                    <h4 style="font-size: 15px; font-weight: 600; color: #333; text-align:center; max-width: 100%; white-space: normal; line-height: 1.3;"><?php echo htmlspecialchars($c['title']); ?></h4>
                  </a>
                <?php endforeach; ?>
              </div>
            </div>
            ...
            <script>
            document.addEventListener('DOMContentLoaded', function() {
                const scrollContainer = document.getElementById('concernAutoScroll');
                ...
                function autoScroll() {
                    if (!isPaused) {
                        scrollContainer.scrollLeft += scrollSpeed;
                        // Reset if reached halfway (since we duplicated content)
            ...
            <div class="wide-banner">
                <div class="wide-slider" id="center-slider" style="position:relative; width:100%; height:100%; overflow:hidden;">
                ...
                <script>
                  document.addEventListener('DOMContentLoaded', function() {
                    const container = document.getElementById('center-slider');
                    if (!container) return; // specific container check
            ...
            <div class="top-seasonals-section" style="margin-top: 40px; margin-bottom: 20px;">
              ...
              <div id="seasonalAutoScroll" style="display: flex; gap: 30px; overflow-x: auto; padding: 10px 4px; -webkit-overflow-scrolling: touch;">
                ...
                ?>
                  <a href="product.php?seasonal=<?php echo urlencode($s['slug']); ?>" style="flex: 0 0 calc(33.333% - 20px); text-decoration: none; color: inherit; display: flex; flex-direction: column; align-items: center; gap: 12px; scroll-snap-align: start;">
                    <div style="width: 100%; aspect-ratio: 1 / 1; overflow: hidden; border: 3px solid #fff; box-shadow: 0 4px 10px rgba(0,0,0,0.1); background: #fff; position: relative; padding: 10px;">
                    ...
                    <h4 style="font-size: 15px; font-weight: 600; color: #333; text-align:center; max-width: 100%; white-space: normal; line-height: 1.3;"><?php echo htmlspecialchars($s['title']); ?></h4>
                  </a>
                <?php endforeach; ?>
              </div>
            </div>
             ...
             <script>
            document.addEventListener('DOMContentLoaded', function() {
                const seasonalContainer = document.getElementById('seasonalAutoScroll');
                if (!seasonalContainer) return;

                // Clone children for seamless looping
                const children = Array.from(seasonalContainer.children);
                children.forEach(child => {
                    let clone = child.cloneNode(true);
                ...
                function autoScrollSeasonal() {
                    if (!isPaused) {
                        seasonalContainer.scrollLeft += scrollSpeed;
                        if (seasonalContainer.scrollLeft >= (seasonalContainer.scrollWidth / 2)) {
                            seasonalContainer.scrollLeft = 0;
                        }
                    }
                    requestAnimationFrame(autoScrollSeasonal);
                }

                seasonalContainer.addEventListener('mouseenter', () => isPaused = true);
                seasonalContainer.addEventListener('mouseleave', () => isPaused = false);
            ...
            <!-- SHOP BY HERBAL (New Section) -->
            <?php if (!empty($herbals)): ?>
            <div class="top-herbals-section" style="margin-top: 50px; margin-bottom: 20px;">
              <div class="section-header">
                <h3>Shop by Herbal</h3>
              </div>
              
              <!-- GRID CONTAINER (Using same visual style as Seasonals/Concern - 3 items per view) -->
              <style>
                #herbalAutoScroll::-webkit-scrollbar { display: none; }
                #herbalAutoScroll { -ms-overflow-style: none; scrollbar-width: none; }
              </style>
              <div id="herbalAutoScroll" style="display: flex; gap: 30px; overflow-x: auto; padding: 10px 4px; -webkit-overflow-scrolling: touch;">
                <?php foreach ($herbals as $h): 
                    $hImg = !empty($h['image']) ? $h['image'] : '/assets/images/category-placeholder.jpg';
                    // Parse Image Path
                     if (strpos($hImg, 'http') === 0) {
                        // is URL
                    } elseif (strpos($hImg, '/') === 0) {
                        // is absolute
                    } elseif (strpos($hImg, 'assets/') === 0) {
                        $hImg = '/' . $hImg;
                    } else {
                        $hImg = '/assets/uploads/herbals/' . $hImg;
                    }
                ?>
                  <a href="product.php?herbal=<?php echo urlencode($h['slug']); ?>" style="flex: 0 0 calc(33.333% - 20px); text-decoration: none; color: inherit; display: flex; flex-direction: column; align-items: center; gap: 12px; scroll-snap-align: start;">
                    <div style="width: 100%; aspect-ratio: 1 / 1; border-radius: 50%; overflow: hidden; border: 3px solid #fff; box-shadow: 0 4px 10px rgba(0,0,0,0.1); background: #fff; position: relative; padding: 10px;">
                         <img src="<?php echo htmlspecialchars($hImg, ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars($h['title'], ENT_QUOTES, 'UTF-8'); ?>" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%; transition: transform 0.3s;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                    </div>
                    <h4 style="font-size: 15px; font-weight: 600; color: #333; text-align:center; max-width: 100%; white-space: normal; line-height: 1.3;"><?php echo htmlspecialchars($h['title']); ?></h4>
                  </a>
                <?php endforeach; ?>
              </div>
            </div>
            ...
            <script>
            document.addEventListener('DOMContentLoaded', function() {
                const herbalContainer = document.getElementById('herbalAutoScroll');
                if (!herbalContainer) return;

                // Clone children for seamless looping
                const children = Array.from(herbalContainer.children);
                children.forEach(child => {
                    let clone = child.cloneNode(true);
                    herbalContainer.appendChild(clone);
                });

                let scrollSpeed = 1; 
                let isPaused = false;

                function autoScrollHerbal() {
                    if (!isPaused) {
                        herbalContainer.scrollLeft += scrollSpeed;
                        if (herbalContainer.scrollLeft >= (herbalContainer.scrollWidth / 2)) {
                            herbalContainer.scrollLeft = 0;
                        }
                    }
                    requestAnimationFrame(autoScrollHerbal);
                }

                herbalContainer.addEventListener('mouseenter', () => isPaused = true);
                herbalContainer.addEventListener('mouseleave', () => isPaused = false);
  ...
  <?php
  // Fetch featured INTERNAL reviews
  $featReviews = [];
  try {
      $featReviews = $pdo->query("SELECT r.*, p.name as product_name, p.images as product_images 
  ...
  // Fetch featured EXTERNAL reviews
  $extReviews = [];
  try {
      $extReviews = $pdo->query("SELECT * FROM external_reviews WHERE is_active = 1 AND is_featured = 1 ORDER BY sort_order ASC, created_at DESC LIMIT 10")->fetchAll(PDO::FETCH_ASSOC);
  } catch (Exception $e) { }
  ...
  // Add type 'internal'
  foreach($featReviews as $fr) {
      $fr['review_type'] = 'internal';
      $allReviews[] = $fr;
  }
  // Add type 'external'
  foreach($extReviews as $er) {
      $er['review_type'] = 'external';
      $allReviews[] = $er;
  }
  ...
  <section class="section-reviews" style="background:linear-gradient(to bottom, #ffffff, #f9fafb); padding:70px 0;">
    <div class="container">
      <div style="text-align:center; margin-bottom:50px;">
        ...
        <p style="font-size:15px; color:#666; margin:0;">Real reviews from verified purchases & across the web</p>
      </div>
        ...
        <?php foreach ($allReviews as $rev): ?>
          <div class="home-review-card">
            ...
            <div class="review-stars">
              <?= str_repeat('<i class="fa-solid fa-star"></i>', (int)$rev['rating']) ?>
              <?= str_repeat('<i class="fa-regular fa-star"></i>', 5 - (int)$rev['rating']) ?>
            </div>
            
            <?php 
                // Content logic
                $content = $rev['review_type'] === 'internal' ? $rev['comment'] : $rev['review_content'];
                $reviewer = $rev['review_type'] === 'internal' ? ($rev['reviewer_name'] ?: 'Verified Customer') : $rev['reviewer_name'];
            ?>

            <p class="review-text">
              "<?= htmlspecialchars(mb_strimwidth($content, 0, 140, '...')) ?>"
            </p>
            
            <div class="review-meta">
              <?php if ($rev['review_type'] === 'internal'): 
                   $prodImg = get_first_image($rev['product_images']);
              ?>
                  <a href="product_view.php?id=<?= $rev['product_id'] ?>" class="review-prod-img">
                    <img src="<?= htmlspecialchars($prodImg) ?>" alt="Product">
                  </a>
                  <div class="review-info">
                    <div class="reviewer-name">
                      <?= htmlspecialchars($reviewer) ?>
                      <i class="fa-solid fa-circle-check" style="color:#2ECC71; margin-left:4px; font-size:12px;"></i>
                    </div>
                    <div class="reviewed-product">
                      Verified Purchase • <a href="product_view.php?id=<?= $rev['product_id'] ?>"><?= htmlspecialchars($rev['product_name']) ?></a>
                    </div>
                  </div>

              <?php else: // External 
                  $extImg = !empty($rev['product_image']) ? $rev['product_image'] : $rev['platform_icon'];
                  $hasProdImg = !empty($rev['product_image']);
              ?>
                  <div class="review-prod-img" style="background:#fff; padding:5px; border:1px solid #eee;">
                    <?php if(!empty($extImg)): ?>
                        <img src="<?= htmlspecialchars($extImg) ?>" alt="<?= htmlspecialchars($rev['platform_name']) ?>" style="object-fit:contain;">
                    <?php else: ?>
                  ...
                  <div class="review-info">
                    ...
                    <div class="reviewed-product">
                      <?php if (!empty($rev['product_name'])): ?>
                          Verified Purchase • <strong><?= htmlspecialchars($rev['product_name']) ?></strong>
                          <span style="display:block; font-size:10px; color:#999;">via <?= htmlspecialchars($rev['platform_name']) ?></span>
                      <?php else: ?>
                          Review on <strong><?= htmlspecialchars($rev['platform_name']) ?></strong>
                          <?php if(!empty($rev['review_link'])): ?>
                             • <a href="<?= htmlspecialchars($rev['review_link']) ?>" target="_blank">View</a>
                          <?php endif; ?>
                      <?php endif; ?>
                    </div>
                  </div>
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        if (reviewsContainer) {
          ...
          reviewsContainer.addEventListener('touchstart', () => {
            ...
            reviewsContainer.resumeTimeout = setTimeout(() => isPaused = false, 5000);
          });
          reviewsContainer.addEventListener('scroll', () => {
             // Optional: pause on manual scroll too
             isPaused = true;
             clearTimeout(reviewsContainer.resumeTimeout);
             reviewsContainer.resumeTimeout = setTimeout(() => isPaused = false, 5000);
          });
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  <!-- Auto-scroll for Product Grids on Mobile (Infinite Loop) -->
  <script>
    window.addEventListener("load", function() {
      const grids = document.querySelectorAll('.product-grid');
      
      grids.forEach(grid => {
        // Check if it's in mobile flex mode
        if (getComputedStyle(grid).display !== 'flex') return;
        ...
        function autoScrollGrid() {
            if (!isHovered) {
                ...
                // If we scrolled past the original width, reset seamlessly
                // Using scrollWidth / 2 assumes we doubled the content
